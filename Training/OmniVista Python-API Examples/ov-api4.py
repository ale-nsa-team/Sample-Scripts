...
######## An example on how to connect to OV and pull Hardware Inventory for a device using oAuth2
...
if login_response != "login.success":
    ...
    if response_status != "SUCCESS":
     ...
     for device in api_response_json['response']:
         ...
         api_domain="/hardware-inventory?deviceIds=" + device_id
         api_request=requests.Request('GET', api_url + api_domain, headers=api_call_headers)
         api_response = ov_session.send(ov_session.prepare_request(api_request), verify=False)
         api_inventory_json=api_response.json()
         for inventory in api_inventory_json['response']:
              print ("Serial Number-->" + inventory['serialNum'] + " ,Type--> " + inventory['moduleType']  + ", Description-->  " + inventory['physicalName'] + ", Uboot -->  " + inventory['ubootVersion'] )
